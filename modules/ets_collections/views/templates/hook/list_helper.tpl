...
var text_update_position='{l s='Successfully updated' mod='ets_collections'}';
</script>
<div class="panel ets_collection-panel{if isset($class)} {$class|escape:'html':'UTF-8'}{/if}">
    <div class="panel-heading">{*if isset($icon) && $icon}<i class="{$icon|escape:'html':'UTF-8'}"></i>&nbsp;{/if*}{$title nofilter}
        ...
        <span class="panel-heading-action">
            {if isset($setting_link) && $setting_link}            
                <a class="list-toolbar-btn" href="{$setting_link|escape:'html':'UTF-8'}">
                    <span data-placement="top" data-html="true" data-original-title="{l s='Settings' mod='ets_collections'}" class="label-tooltip" data-toggle="tooltip" title="{l s='Settings' mod='ets_collections'}">
        				<i class="icon-cog"></i>
                    </span>
                </a>            
            {/if}
            {if isset($show_add_new) && $show_add_new}            
                <a class="list-toolbar-btn add_new_link" href="{if isset($link_new)}{$link_new|escape:'html':'UTF-8'}{else}{$currentIndex|escape:'html':'UTF-8'}{/if}">
                    <span data-placement="top" data-html="true" data-original-title="{l s='Add new' mod='ets_collections'}" class="label-tooltip" data-toggle="tooltip" title="">
        				<i class="process-icon-new"></i> {l s='Add new' mod='ets_collections'}
                    </span>
                </a>            
            {/if}
        </span>
            ...
            <form method="post" action="{$currentIndex|escape:'html':'UTF-8'}">
                ...
                <table class="table configuration alltab_ss{if isset($has_delete_product) && (Configuration::get('ETS_MP_ALLOW_SELLER_CREATE_PRODUCT') || (isset($has_edit_product) && $has_edit_product) || $has_delete_product) } allow_checkbox_product{/if} list-{$name|escape:'html':'UTF-8'}">
                    <thead>
                        <tr class="nodrag nodrop">
                            ...
                            {foreach from=$fields_list item='field' key='index'}
                                <th class="{if isset($index) && $index == 'active'}ets_col_{/if}{$index|escape:'html':'UTF-8'}{if isset($field.class)} {$field.class|escape:'html':'UTF-8'}{/if}" {if $show_action && !$actions && count($fields_list)==$i}colspan="2"{/if}>
                                    <span class="title_box">
                            ...
                            {if $show_action && $actions}
                                <th class="table_action" style="text-align: right;">{l s='Action' mod='ets_collections'}</th>
                            {/if}
                        </tr>
                        {if $show_toolbar}
                            <tr class="nodrag nodrop filter row_hover">
                                ...
                                {/if}
                                {foreach from=$fields_list item='field' key='index'}
                                    <th class="{if isset($index) && $index == 'active'}ets_col_{/if}{$index|escape:'html':'UTF-8'}{if isset($field.class)} {$field.class|escape:'html':'UTF-8'}{/if}">
                                        {if isset($field.filter) && $field.filter}
                                            ...
                                            {/if}
                                            {if $field.type=='int'}
                                                <label for="{$index|escape:'html':'UTF-8'}_min"><input type="text" placeholder="{l s='Min' mod='ets_collections'}" name="{$index|escape:'html':'UTF-8'}_min" value="{$field.active.min|escape:'html':'UTF-8'}" /></label>
                                                <label for="{$index|escape:'html':'UTF-8'}_max"><input type="text" placeholder="{l s='Max' mod='ets_collections'}" name="{$index|escape:'html':'UTF-8'}_max" value="{$field.active.max|escape:'html':'UTF-8'}" /></label>
                                            {/if}
                                            {if $field.type=='date'}
                                                <fieldset class="form-group"> 
                                                    <div class="input-group ets_col_datepicker">
                                                        <input id="{$index|escape:'html':'UTF-8'}_min" autocomplete="off" class="form-control" name="{$index|escape:'html':'UTF-8'}_min" placeholder="{l s='From' mod='ets_collections'}" value="{$field.active.min|escape:'html':'UTF-8'}" type="text" autocomplete="off" />
                                                ...
                                                <fieldset class="form-group"> 
                                                    <div class="input-group ets_col_datepicker">
                                                        <input id="{$index|escape:'html':'UTF-8'}_max" autocomplete="off" class="form-control" name="{$index|escape:'html':'UTF-8'}_max" placeholder="{l s='To' mod='ets_collections'}" value="{$field.active.max|escape:'html':'UTF-8'}" type="text" autocomplete="off" />
                                                        <div class="input-group-append">
                                                            <div class="input-group-text">
                                                                <i class="icon icon-date"></i>
                                                            </div>
                                ...
                                {if $show_action}
                                    <th class="actions">
                                        <span class="pull-right flex">
                                            ...
                                            {if $show_reset}<a  class="btn btn-warning"  href="{$currentIndex|escape:'html':'UTF-8'}"><i class="icon-eraser"></i> {l s='Reset' mod='ets_collections'}</a> &nbsp;{/if}
                                            <button class="btn btn-default" name="ets_col_submit_{$name|escape:'html':'UTF-8'}" id="ets_col_submit_{$name|escape:'html':'UTF-8'}" type="submit">
            									<i class="icon-search"></i> {l s='Filter' mod='ets_collections'}
            								</button>
                                        </span>
                                    </th>
                                {/if}
                    ...
                    <tbody id="list-{$name|escape:'html':'UTF-8'}">
                        ...
                        {foreach from=$field_values item='row'}
                            <tr id="{$name|escape:'html':'UTF-8'}-{$row.$identifier|intval}" {if isset($row.read) && !$row.read}class="no-read"{/if} data-id="{$row.$identifier|intval}">
                                ...
                                {foreach from=$fields_list item='field' key='key'}                             
                                    <td class="{if isset($key) && $key == 'active'}ets_col_{/if}{$key|escape:'html':'UTF-8'} {if isset($sort)&& $sort==$key && isset($sort_type) && $sort_type=='asc' && isset($field.update_position) && $field.update_position}pointer dragHandle center{/if}{if isset($field.class)} {$field.class|escape:'html':'UTF-8'}{/if}" {if $show_action && !$actions && count($fields_list)==$i}colspan="2"{/if} >
                                        ...
                                        {else}
                                            {if isset($row.$key) && $row.$key}
                                                <a name="{$name|escape:'html':'UTF-8'}"  href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&change_enabled=0&field={$key|escape:'html':'UTF-8'}" class="list-action field-{$key|escape:'html':'UTF-8'} list-action-enable action-enabled list-item-{$row.$identifier|escape:'html':'UTF-8'}" data-id="{$row.$identifier|escape:'html':'UTF-8'}" title="{l s='Click to turn off' mod='ets_collections'}">
                                                    <span class="action_field_content">
                                                        <i class="fa fa-check"></i>
                                                    </span>
                                                    <span class="action_yes">{l s='On' mod='ets_collections'}</span>
                                                    <span class="action_no">{l s='Off' mod='ets_collections'}</span>
                                                </a>
                                            {else}
                                                <a name="{$name|escape:'html':'UTF-8'}" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&change_enabled=1&field={$key|escape:'html':'UTF-8'}" class="list-action field-{$key|escape:'html':'UTF-8'} list-action-enable action-disabled  list-item-{$row.$identifier|escape:'html':'UTF-8'}" data-id="{$row.$identifier|escape:'html':'UTF-8'}" title="{l s='Click to turn on' mod='ets_collections'}">
                                                    <span class="action_field_content">
                                                ...
                                                <div class="btn-group pull-right">
                                                    {if $actions[0]=='view'}
                                                        {if isset($row.child_view_url) && $row.child_view_url}
                                                            <a class="btn btn-default link_view" href="{$row.child_view_url|escape:'html':'UTF-8'}" {if isset($view_new_tab) && $view_new_tab} target="_blank" {/if}><i class="icon-search-plus fa fa-search-plus"></i> {l s='View' mod='ets_collections'}</a>
                                                        {elseif !isset($row.action_edit) || $row.action_edit}
                                                            <a class="btn btn-default link_edit" href="{$currentIndex|escape:'html':'UTF-8'}&edit{$name|escape:'html':'UTF-8'}=1&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}" ><i class="icon-pencil fa fa-pencil"></i> {l s='Edit' mod='ets_collections'}</a>
                                                        {/if}
                                                    {/if}
                                                    {if $actions[0]=='delete'}
                                                        <a class="btn btn-default" onclick="return confirm('{l s='Do you want to delete this item?' mod='ets_collections' js=1}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes"><i class="icon-trash fa fa-trash"></i> {l s='Delete' mod='ets_collections'}</a>
                                                    {/if}
                                                            ...
                                                            {foreach from=$actions item='action' key='key'}
                                                                {if $key!=0}
                                                                    {if $action=='delete' && (!isset($row.view_order_url) || (isset($row.view_order_url) && !$row.view_order_url) )}
                                                                        <li><a class="btn btn-default" onclick="return confirm('{if $name=='mp_front_products' || $name=='mp_products'}{l s='Do you want to delete this product?' mod='ets_collections'}{else}{l s='Do you want to delete this item?' mod='ets_collections'}{/if}');" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}&del=yes{if isset($row.type)}&type={$row.type|escape:'html':'UTF-8'}{/if}"><i class="fa fa-trash icon-trash"></i> {l s='Delete' mod='ets_collections'}</a></li>
                                                                    {/if}
                                                                    {if $action=='view'}
                                                                        {if isset($row.child_view_url) && $row.child_view_url}
                                                                            <li><a class="btn btn-default" href="{$row.child_view_url|escape:'html':'UTF-8'}"><i class="fa fa-search-plus icon-search-plus"></i> {l s='View' mod='ets_collections'}</a></li>
                                                                        {else}
                                                                            <li><a class="btn btn-default" href="{$currentIndex|escape:'html':'UTF-8'}&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="fa fa-pencil icon-pencil"></i> {l s='Edit' mod='ets_collections'}</a></li>
                                                                        {/if}
                                                                    {/if}
                                                                    {if $action =='edit'}
                                                                        <li><a class="btn btn-default" href="{$currentIndex|escape:'html':'UTF-8'}&edit{$name|escape:'html':'UTF-8'}=1&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="fa fa-pencil icon-pencil"></i> {l s='Edit' mod='ets_collections'}</a></li>
                                                                    {/if}
                                                                    {if $action =='duplicate'}
                                                                        <li><a class="btn btn-default" href="{$currentIndex|escape:'html':'UTF-8'}&duplicate{$name|escape:'html':'UTF-8'}=1&{$identifier|escape:'html':'UTF-8'}={$row.$identifier|escape:'html':'UTF-8'}"><i class="fa fa-copy icon-copy"></i> {l s='Duplicate' mod='ets_collections'}</a></li>
                                                                    {/if}
                                                                {/if}
                                                            {/foreach}
                                                        </ul>
                        ...
                        {/if}  
                        {if !$field_values}
                           <tr class="no-record not_items_found"> <td colspan="100%"><p>{l s='No items found' mod='ets_collections'}</p></td></tr> 
                        {/if}                
                    </tbody>
                </table>
                {if isset($bulk_actions) && $bulk_actions}
                    <select id="bulk_action_{$name|escape:'html':'UTF-8'}" name="bulk_action_{$name|escape:'html':'UTF-8'}" style="display:none;">
                        <option value="">{l s='Bulk actions' mod='ets_collections'}</option>
                        <option value="delete_all">{l s='Delete all selected' mod='ets_collections'}</option>
                        <option value="duplicate_all">{l s='Duplicate all selected' mod='ets_collections'}</option>
                    </select>
                {/if}
                {if $paggination}
    ...
    {if isset($link_back_to_list)}
        <div class="panel-footer">
            <a id="desc-attribute-back" class="btn btn-default btn-primary" href="{$link_back_to_list|escape:'html':'UTF-8'}">
        		<i class="process-icon-back "></i> <span>{l s='Back to list' mod='ets_collections'}</span>
        	</a>
        </div>
    {/if}
</div>