                    ...
                    if($comment->update())
                    {
                        ...
                        if($commentapproved && ($subject = Ybc_blog_email_template_class::getSubjectByTemplate('approved_comment',$idLang)))
                        {
                            ...
                            Mail::Send(
                                ...
                                'approved_comment',
                                $subject,
                                array('{customer_name}' => $comment->name, '{email}' => $comment->email,'{rating}' => ' '.($comment->rating != 1 ? $this->module->l('stars','blog') : $this->module->l('star','blog')), '{subject}' => $comment->subject, '{comment}'=>$comment->comment,'{post_title}'=>$post->title,'{post_link}' => $this->module->getLink('blog', array('id_post' => $comment->id_post)),'{color_main}'=>Configuration::get('YBC_BLOG_CUSTOM_COLOR'),'{color_hover}'=>Configuration::get('YBC_BLOG_CUSTOM_COLOR_HOVER')),
            ...
            if(!$this->_errros)
            {
                if($id_employee_post)
                {
                    ...
                    if(!$employeePost->add())
                        ...
                        $this->sussecfull = $this->module->l('Information updated');
