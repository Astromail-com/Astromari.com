...
class Ets_affiliatemarketingVoucherModuleFrontController extends  Ets_affiliatemarketingAllModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (Tools::isSubmit('eam_apply_voucher')) {
            ...
            $sql = "SELECT COUNT(*) as total FROM `" . _DB_PREFIX_ . "ets_am_reward_usage` WHERE id_voucher = " . (int)$cart_rule->id . " AND id_customer = " . (int)$this->context->customer->id;
                ...
                if (count($errors)) {
                    ...
                    if ($cart_rule->add()) {
                        ...
                        if($usageLOY > 0){
                            ...
                            $rewardUsage->save();
                            Ets_affiliatemarketing::loyRewardUsed($usageLOY,$rewardUsage->id,$rewardUsage->id_customer);
                        }
                        if($usageANR > 0){
    ...
    protected function getVoucherConfiguration()
    {
        ...
        if ($max = Configuration::get('ETS_AM_MAX_BALANCE_REQUIRED_FOR_VOUCHER')) {
            $max = (float)$max;
        } else {
            $max = 0;
        }
