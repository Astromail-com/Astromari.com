        ...
        if ($this->valid) {
            ...
            if ($id_product = (int)Tools::getValue('id_product')) {
                ...
                FROM `" . _DB_PREFIX_ . "ets_am_reward_product` rp
                      LEFT JOIN `"._DB_PREFIX_."product_lang` pl ON pl.id_product = rp.id_product AND pl.id_lang ='".(int)$this->context->language->id."'
                      LEFT JOIN `" . _DB_PREFIX_ . "ets_am_reward` r ON r.id_ets_am_reward = rp.id_ets_am_reward
                WHERE rp.id_seller = " . (int)$this->context->customer->id . "
                      AND r.id_shop = " . (int)$this->context->shop->id . "
                      AND r.program = '" . pSQL(EAM_AM_AFFILIATE_REWARD) . "'
                      AND rp.id_product = " . (int)$id_product;
                $product_sale = Db::getInstance()->getRow($sql);
