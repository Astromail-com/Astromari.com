...
class Ets_affiliatemarketingRegisterModuleFrontController extends Ets_affiliatemarketingAllModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_user` WHERE '.pSQL($_program).'=-1 AND id_customer='.(int)$id_customer))
                $alert_type = 'program_banned';
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_user` WHERE '.pSQL($_program).'=1 AND id_customer='.(int)$id_customer))
                $alert_type = 'registered';
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_user` WHERE '.pSQL($_program).'=-2 AND id_customer='.(int)$id_customer))
                $alert_type = 'program_decline';
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_participation` WHERE program ="'.pSQL($_program).'" AND status=0 AND id_customer='.(int)$id_customer))
            $alert_type = 'register_success';
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_participation` WHERE program ="'.pSQL($_program).'" AND status=1 AND id_customer='.(int)$id_customer))
            $alert_type = 'registered';
        elseif(Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_am_participation` WHERE program ="'.pSQL($_program).'" AND status < 0 AND id_customer='.(int)$id_customer))
            $alert_type = 'program_banned';    
        if(!$alert_type){
            ...
            elseif($_program == 'loy'){
                if(!Ets_Loyalty::isLoyaltyEnabled()){
                     $alert_type = 'disabled';
                }
        ...
        if($alert_type == 'need_condition'){
            ...
            elseif($_program == 'loy'){
                if(!Ets_Loyalty::isCustomerBelongToLoyaltyGroup($this->context)){
                    $message = '';
                    if(!$message){
                        Tools::redirect($this->context->link->getPageLink('my-account', true));
                    }
