...
class Ets_Loyalty extends Ets_AM
{
     ...
     * @return bool
     */
    public static function isLoyaltyEnabled()
    {
        return self::isEnable('ETS_AM_LOYALTY_ENABLED');
    }
    /**
     * @return bool
     * @throws PrestaShopDatabaseException
     ...
     * @return bool
     */
    public static function isCustomerBelongToLoyaltyGroup()
    {
        $customer = Context::getContext()->customer;
        return self::isCustomerBelongToValidGroup($customer, 'ETS_AM_LOYALTY_GROUPS');
    }
    /**
     * @param $cart
     * @return null|string
    ...
    public static function getCartMessage($cart)
    {
        ...
        if (!Ets_Loyalty::isCustomerCanJoinLoyaltyProgram() || !Ets_Loyalty::isCustomerJoinedLoyaltyProgram($context->customer->id) || !$cart || !$cart->id) {
            return '';
        }
        ...
        if ($cart_reward > 0 && $min_loyalty != '' && (float)$min_loyalty > ($cart_total = (float)$cart->getOrderTotal(!Configuration::get('ETS_AM_LOYALTY_EXCLUDE_TAX')))) {
    ...
    public static function sendEmailToAdminWhenRewardValidated($reward)
    {
        ...
        if ($customer) {
            ...
            $data = array(
                '{title}' => Ets_affiliatemarketing::$trans['reward_validated'],
                '{customer}' => $customer->firstname . ' ' . $customer->lastname,
                '{reward}' => $reward->id,
                '{amount}' => Ets_affiliatemarketing::displayPrice($reward->amount, (int)Configuration::get('PS_CURRENCY_DEFAULT')),
                '{type}' => $reward->program,
    ...
    public static function sendEmailToCustomerWhenRewardExpired($reward)
    {
        ...
        if ($customer && Validate::isLoadedObject($customer)) {
            $data = array(
                '{customer}' => $customer->firstname . ' ' . $customer->lastname,
                '{title}' => Ets_affiliatemarketing::$trans['reward_expired'],
                '{amount}' => Ets_affiliatemarketing::displayPrice($reward->amount, (int)Configuration::get('PS_CURRENCY_DEFAULT')),
                '{type}' => $reward->program,
                '{reward_id}' => $reward->id,
                '{reward}' => $reward->id,
    ...
    public static function loyaltyProductReward($product, $type = 'product', $context = null, $get_force_reward = false)
    {
        ...
        if ($cal_by == 'FIXED') {
            ...
            $min_loyalty = Configuration::get('ETS_AM_LOYALTY_MIN');
            $max_loyalty = (float)Configuration::get('ETS_AM_LOYALTY_MAX');
            if ($min_loyalty) {
                $min_loyalty = (float)$min_loyalty * $context->currency->conversion_rate;
            }
    ...
    public static function getCartReward(Cart $cart, $context = null, $need_product = false, $get_force_reward = false)
    {
        ...
        $cart_total = (float)$cart->getOrderTotal(!Configuration::get('ETS_AM_LOYALTY_EXCLUDE_TAX')) - (float)$cart->getOrderTotal(!Configuration::get('ETS_AM_LOYALTY_EXCLUDE_TAX'), Cart::ONLY_SHIPPING);
        $min_loyalty = Configuration::get('ETS_AM_LOYALTY_MIN');
        if ($min_loyalty) {
            $min_loyalty = (float)$min_loyalty * $context->currency->conversion_rate;
        }
        if ($min_loyalty != '' && $min_loyalty > $cart_total && !$get_force_reward)
            return 0;
    ...
    public static function getOrderReward(Order $order, $context = null, $need_product = false, $get_force_reward = false)
    {
        ...
        if ($loyalty_product || $cart_products) {
            ...
            /*
            $disCount = $context->cart->getOrderTotal(true, Cart::ONLY_DISCOUNTS);
            if ((float)$disCount) {
                $rwDiscount = $disCount * 0.01 * (float)Configuration::get('ETS_AM_LOYALTY_GEN_PERCENT');
                $cart_reward = $rwDiscount < $cart_reward ? $cart_reward - $rwDiscount : 0;
            }*/
        }
        if ($max_loyalty != '' && $max_loyalty < $cart_reward)
            $cart_reward = $max_loyalty;
    ...
    public static function calculateCartTotalReward($params, $need_product = false)
    {
        ...
        if (!(isset($params['order'])) || !$params['order'] || Ets_Loyalty::isCustomerSuspendedOrBannedLoyaltyProgram($context->customer->id)
            || !Ets_Loyalty::isCustomerCanJoinLoyaltyProgram() || !Ets_Loyalty::isCustomerJoinedLoyaltyProgram($context->customer->id))
            return 0;
        $reward = self::getOrderReward($params['order'], $context, $need_product);
        return $reward;
    }
    /**
     * @param $id_customer
     ...
     * @throws PrestaShopException
     */
    public static function isCustomerJoinedLoyaltyProgram($id_customer, $status = 1)
    {
        return Ets_AM::isCustomerJoinedProgram($id_customer, EAM_AM_LOYALTY_REWARD, $status);
    }
    /**
     * @param $id_customer
     * @return bool|array
