    ...
    public function createPaymentMethodField($params)
    {
        ...
        if ($this->add()) {
            ...
            foreach ($languages as $lang) {
                ...
                $sql_field .= " INSERT INTO `" . _DB_PREFIX_ . "ets_am_payment_method_field_lang` (id_payment_method_field, id_lang, title, description) VALUES(" . (int)$this->id . ", " . (int)$id_lang . ", '" . pSQL(isset($params['title'][$id_lang]) && $params['title'][$id_lang] ? $params['title'][$id_lang] : $default_title) . "', '" . pSQL(isset($params['desc'][$id_lang]) ? $params['desc'][$id_lang] : '') . "');";
            }
            if ($sql_field) {
