...
class EtsAmAdmin extends ObjectModel
{
    ...
    public static function updatePaymentMethod($id_pm, $pm_title = array(), $pm_fee_type = null, $pm_fee_fixed = null, $pm_fee_percent = null, $pm_enable = null, $pm_estimated = null, $pm_desc = array(), $pm_fields = array(), $pm_note = array())
    {
        if ($id_pm) {
            ...
            foreach ($languages as $lang) {
                ...
                if ($exists) {
                    $sql .= "UPDATE `" . _DB_PREFIX_ . "ets_am_payment_method_lang` SET title = '" . pSQL(isset($pm_title[$id_lang]) ? $pm_title[$id_lang] : '') . "', description = '" . pSQL(isset($pm_desc[$id_lang]) ? $pm_desc[$id_lang] : '') . "', note = '" . pSQL(isset($pm_note[$id_lang]) ? $pm_note[$id_lang] : '') . "' WHERE id_payment_method = " . (int)$id_pm . " AND id_lang =" . (int)$id_lang . ";";
                } else {
                    $sql .= "INSERT INTO `" . _DB_PREFIX_ . "ets_am_payment_method_lang` (id_payment_method, title, description, note,id_lang) VALUES (" . (int)$id_pm . ", '" . pSQL(isset($pm_title[$id_lang]) ? $pm_title[$id_lang] : '') . "', '" . pSQL(isset($pm_desc[$id_lang]) ? $pm_desc[$id_lang] : '') . "', '" . pSQL(isset($pm_note[$id_lang]) ? $pm_note[$id_lang] : '') . "', " . (int)$id_lang . ");";
                }
            }
            if ($sql) {
            ...
            if ($pm_fields) {
                ...
                foreach ($pm_fields as $key => $field) {
                    if (isset($field['id']) && $field['id']) {
                        if ($field['type']) {
                            $sql_field .= " UPDATE `" . _DB_PREFIX_ . "ets_am_payment_method_field` SET type = '" . pSQL($field['type']) . "', enable = " . (int)$field['enable'] . ", required = " . (int)$field['required'] . " WHERE id_ets_am_payment_method_field = " . (int)$field['id'] . ";";
                        }
                        $default_title = '';
                        foreach ($languages as $lang) {
                        ...
                        $sql_field .= "INSERT INTO `" . _DB_PREFIX_ . "ets_am_payment_method_field` (id_payment_method, `type`, enable, required) VALUES (" . (int)$id_pm . ", '" . pSQL($field['type']) . "', " . (int)$field['enable'] . ", " . (int)$field['required'] . "); SET @id_pmf_" . pSQL($key) . " = LAST_INSERT_ID();";
                        $default_title = '';
    ...
    public static function createPaymentMethod($pm_title = array(), $pm_fee_type = null, $pm_fee_fixed = null, $pm_fee_percent = null, $pm_enable = null, $pm_estimated = null, $pm_desc = array(), $pm_note = array())
    {
        ...
        foreach ($languages as $lang) {
            ...
            $sql .= "INSERT INTO `" . _DB_PREFIX_ . "ets_am_payment_method_lang` (id_payment_method, title, description, note, id_lang) VALUES (" . (int)$id_pm . ", '" . pSQL($title) . "', '" . pSQL($desc) . "', '" . pSQL($note) . "'," . (int)$lang['id_lang'] . ");";
        }
        if ($sql) {
            Db::getInstance()->execute($sql);
        }
    ...
    public static function updateSortPaymentMethod($data)
    {
        if ($data) {
            ...
            foreach ($data as $key => $item) {
                $index = $key + 1;
                $item = (int)$item;
                $sql .= " UPDATE `" . _DB_PREFIX_ . "ets_am_payment_method` SET sort = " . (int)$index . " WHERE id_ets_am_payment_method =" . (int)$item;
            }
            if ($sql) {
                Db::getInstance()->execute($sql);
                return true;
    ...
    public static function updateSortPaymentMethodfield($data)
    {
        if ($data) {
            ...
            foreach ($data as $key => $item) {
                $index = $key + 1;
                $item = (int)$item;
                $sql .= " UPDATE `" . _DB_PREFIX_ . "ets_am_payment_method_field` SET sort = " . (int)$index . " WHERE id_ets_am_payment_method_field =" . (int)$item;
            }
            if ($sql) {
                Db::getInstance()->execute($sql);
                return true;
    ...
    public static function getRewardUsers($nb = false, $filter_where = '', $_orderBy = '', $_orderWay = '', $page = false, $limit = 25, $having = '')
    {
        $context = Context::getContext();
        if ($nb) {
           ...
           GROUP BY reward2.id_customer) reward ON (reward.id_customer=customer.id_customer)
                    LEFT JOIN (
                            SELECT r2.id_customer, 
                                (SUM(IF ( r2.deleted = 0 AND r2.`status` = 1, r2.amount, 0)) - COALESCE(ru2.reward_balance, 0)) AS reward_balance,
                                (SUM(IF(r2.program='loy' AND r2.deleted=0 AND r2.`status`=1, r2.amount, 0)) - COALESCE(ru2.loy_rewards,0)) AS loy_rewards,
                                (SUM(IF(r2.program='ref' AND r2.deleted=0 AND r2.`status`=1, r2.amount, 0)) - COALESCE(ru2.ref_rewards,0)) AS ref_rewards,
                                (SUM(IF(r2.program='aff' AND r2.deleted=0 AND r2.`status`=1, r2.amount, 0)) - COALESCE(ru2.aff_rewards,0)) AS aff_rewards,
                                (SUM(IF(r2.program='mnu' AND r2.deleted=0 AND r2.`status`=1, r2.amount, 0)) - COALESCE(ru2.mnu_rewards,0)) AS mnu_rewards,
                                ru2.total_withdraws
                            FROM `" . _DB_PREFIX_ . "ets_am_reward` r2 
                            LEFT JOIN (
                                SELECT 
                                    ruu2.id_customer,
                                    SUM(IF(ruu2.`status` != 0 AND ruu2.deleted=0, ruu2.amount, 0)) as reward_balance,
                                    SUM(IF(ruu2.type='loy' AND ruu2.`status`!=0 AND ruu2.deleted=0, ruu2.amount,0)) as loy_rewards,
                                    SUM(IF(ruu2.type='ref' AND ruu2.`status`!=0 AND ruu2.deleted=0, ruu2.amount,0)) as ref_rewards,
                                    SUM(IF(ruu2.type='aff' AND ruu2.`status`!=0 AND ruu2.deleted=0, ruu2.amount,0)) as aff_rewards,
                                    SUM(IF(ruu2.type='mnu' AND ruu2.`status`!=0 AND ruu2.deleted=0, ruu2.amount,0)) as mnu_rewards,
                                    SUM(IF(ruu2.id_withdraw>0 AND ruu2.`status` IN(1,0) AND ruu2.deleted=0,ruu2.amount,0)) AS total_withdraws
                                FROM `" . _DB_PREFIX_ . "ets_am_reward_usage` as ruu2
                                GROUP BY ruu2.id_customer
                             ) ru2 ON r2.id_customer = ru2.id_customer
