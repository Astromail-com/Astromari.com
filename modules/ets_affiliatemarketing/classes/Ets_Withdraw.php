...
class Ets_Withdraw extends ObjectModel
{
                ...
                foreach ($requests as $result) {
                    ...
                    $actions = array(
                        array(
                            ...
                            'link' => $id_customer ? '' : $context->link->getAdminLink('AdminModules', true) . '&configure=ets_affiliatemarketing&tabActive=withdraw_list&id_withdrawal=' . (int)$result['id_ets_am_withdrawal'] . '&view=1',
                            'icon' => 'search',
                            'class' => '',
                            'action' => '',
                            'id' => '',
                    ...
                    if ($result['withdrawal_status'] == 0) {
                        ...
                        $actions[] = array(
                            'label' => Ets_affiliatemarketing::$trans['Decline_return'],
                        ...
                        $actions[] = array(
                            'label' => Ets_affiliatemarketing::$trans['Decline_deduct'],
                    ...
                    if ($result['estimated_time'] && Validate::isInt($result['estimated_time']) && (int)$result['estimated_time'] > 0) {
                        $estimate = strtotime((int)$result['estimated_time'] . ' days');
                        $result['date_process'] = date('Y-m-d', $estimate) . '(' . Ets_affiliatemarketing::$trans['estimated'] . ')';
                    }
                    $results[] = $result;
                }
            }
    ...
    public static function updateWithdrawAndReward($id_usage, $action)
    {
        ...
        $actions = array(
            array(
                'label' => Ets_affiliatemarketing::$trans['View'],
                'href' => $context->link->getAdminLink('AdminModules', true) . '&configure=ets_affiliatemarketing&tabActive=withdraw_list&id_withdrawal=' . (int)$usage->id_withdraw . '&view=1',
                'icon' => 'search',
                'class' => '',
                'action' => '',
                'id' => '',
        ...
        if ($action == 'APPROVE') {
            ...
            $w_status = -1;
            $u_status = 1;
            $note = Ets_affiliatemarketing::$trans['deduct_reward'];
        } elseif ($action == 'DECLINE_RETURN') {
            $w_status = -1;
            $u_status = 0;
            $note = Ets_affiliatemarketing::$trans['return_reward'];
