...
{block name="input_row"}
    ...
    {elseif $input.name == 'ETS_AM_REF_ENABLED_MULTI_LEVEL'}
        ...
        <div class="form-group">
            <button type="button"
                    class="btn btn-default btn-add-level" name="ETS_AM_ADD_LEVEL"><i
                        class="fa fa-plus"></i> {l s='Add level' mod='ets_affiliatemarketing'}</button>
        </div>
...
{block name="input_row"}
    ...
    {if $input.name=='ETS_AM_REF_MAX_INVITATION'}
        <div class="form-group ">
            ...
            <div class="col-lg-5">
                <button class="btn btn-default clear_qr_code_cache"><i class="fa fa-trash"></i> {l s='Clear' mod='ets_affiliatemarketing'}</button>
            </div>
        </div>
    {/if}
{/block}
        ...
        {if !empty($checkboxs_group)}
            ...
            <div class="row">
                ...
                {if $input.name == 'ETS_AM_WAITING_STATUS' || $input.name == 'ETS_AM_VALIDATED_STATUS' || $input.name == 'ETS_AM_CANCELED_STATUS'}
                    <div class="col-md-2">
                        <button type="button" class="btn btn-default js-eam-toggle-states-payment states-hide"><i
    ...
    {elseif $input.type == 'file' && isset($input.is_image) && $input.is_image}
        ...
        {if $fields_value[$input.name] }
            <img src="{$path_banner|escape:'html':'UTF-8'}{$fields_value[$input.name] nofilter}?time={time()|escape:'html':'UTF-8'}" alt="{$fields_value[$input.name]|escape:'html':'UTF-8'}"
                 class="image-preview">
            <button type="button" class="eam-btn-delete-file js-eam-btn-delete-file" data-name="{$input.name|escape:'html':'UTF-8'}"><i class="fa fa-trash"></i></button>
        {/if}
