...
jQuery(document).ready(function ($) {
    ...
    if ($('form').length > 0) {
        //Init
        if($('#ETS_AM_LOYALTY_SPECIFIC_SEARCH').length > 0){
            eamInitProductAdded(['#ETS_AM_LOYALTY_SPECIFIC_SEARCH', '#ETS_AM_LOYALTY_EXCLUDED_SEARCH'], ets_snw_link_ajax,'.snw_products_added');
        }
        if($('#ETS_AM_AFF_SPECIFIC_PRODUCTS_SEARCH').length > 0){
            eamInitProductAdded(['#ETS_AM_AFF_SPECIFIC_PRODUCTS_SEARCH', '#ETS_AM_AFF_PRODUCTS_EXCLUDED_SEARCH'], ets_snw_link_ajax,'.snw_products_added');
        }
        if($('#ETS_AM_REF_REGISTER_PRODUCTS_EXCLUDED_SEARCH').length > 0){
            eamInitProductAdded(['#ETS_AM_REF_REGISTER_PRODUCTS_EXCLUDED_SEARCH','#ETS_AM_REF_PRODUCTS_EXCLUDED_SEARCH'], ets_snw_link_ajax,'.snw_products_added');
        }
            ...
            success: function(res){
                ...
                if(res.success){
                    ...
                    if(action_user_reward == 'active'){
                        ...
                        label_status_view = '<span class="label label-success"><i class="fa fa-check"></i> '+res.status+'</span>';
                    }
                    else if(action_user_reward == 'decline'){
                        label_status = '<span class="label label-default">'+res.status+'</span>';
                        label_status_view = '<span class="label label-default"><i class="fa fa-close"></i> '+res.status+'</span>';
                    }
...
if($('.ets-sn-admin').length > 0){
    ...
    eamToggleFields('input[name=ETS_AM_AFF_ALLOW_CONVERT_TO_VOUCHER]', ['input[name=ETS_AM_MIN_BALANCE_REQUIRED_FOR_VOUCHER]','input[name="ETS_AM_MAX_BALANCE_REQUIRED_FOR_VOUCHER"]','input[name="ETS_AM_AFF_ALLOW_VOUCHER_IN_CART"]']);
   ...
   $('.ets-sn-admin #eamFormActionRewardUser select[name=action]').change(function(event) {
    ...
    $('.ets-sn-admin .payment-method-field .collapse').on('show.bs.collapse', function() {
        $(this).prev('a').find('i').attr('class', 'fa fa-minus-circle pull-right');
    });
    $('.ets-sn-admin .payment-method-field .collapse').on('hide.bs.collapse', function() {
        $(this).prev('a').find('i').attr('class', 'fa fa-plus-circle pull-right');
    });
...
function eamInitProductAdded(array_input, url_ajax, el_list_add) {
    el_search = '';
    if (array_input.length > 0) {
        el_search = array_input[0];
        array_input.shift();
    }
    else {
        return;
    ...
    name_input = $(el_search).attr('data-target');
    if (typeof name_input == 'undefined' || !name_input) {
        return;
    }
    ids_str = $('input[name=' + name_input + ']').val();
    ids_arr = ids_str ? ids_str.split(',') : null;
    if (ids_arr && $(el_search).next().next(el_list_add).length > 0) {
        list_add = $(el_search).next().next(el_list_add);
        $.ajax({
            url: url_ajax,
            data: {
                ids: ids_arr,
                initSearchProduct: true
            },
            type: 'post',
            dataType: 'json',
            success: function (json) {
            ...
            complete: function(){
                eamInitProductAdded(array_input, url_ajax,el_list_add);
            }
        });
    }
    else{
...
function eamRenderFieldsMethodPayment(input, langs, currency) {
    ...
    method_name_html += '<div class="form-group row ">';
        ...
        method_name_html += '</div>';
    method_name_html +=  '</div>';
    method_name_html += '<a class="btn btn-default btn-sm btn-delete-field js-btn-delete-field" href="javascript:void(0)"><i class="fa fa-trash"></i> ' + ets_swn_trans['delete'] + '</a>';
    method_name_html += '</div>';
