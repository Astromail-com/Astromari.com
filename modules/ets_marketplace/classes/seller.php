...
class Ets_mp_seller extends ObjectModel
{
    ...
    )
    {
        ...
        ' . ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '') . '
        ' . ((!$beginning && !$ending) ? ' AND p.`id_product` IN (' . ((Ets_marketplace::validateArray($tab_id_product) && count($tab_id_product)) ? implode(', ', array_map('intvall',$tab_id_product)) : 0) . ')' : '') . (string)$filter . '
        ' . (string)$sql_groups . '
    ...
    public function getTotalTurnOver($id_product=0,$filter=false)
    {
        if(Configuration::get('ETS_MP_COMMISSION_APPROVED_WHEN'))
        {
            ...
            $sql = 'SELECT SUM(od.total_price_tax_incl/c.conversion_rate) FROM `'._DB_PREFIX_.'orders` o
                ...
                WHERE o.id_shop="'.(int)Context::getContext()->shop->id.'" AND o.current_state IN ('.implode(',',array_map('intval',$status)).')'.($filter ? (string)$filter:'').($id_product ? ' AND od.product_id="'.(int)$id_product.'"':'');
