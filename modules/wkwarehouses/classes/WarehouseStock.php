...
class WarehouseStock extends ObjectModel
{
    ...
    public static function getWarehousesAvailableQuantities($id_product, $id_product_attribute = null)
    {
        ...
        $reserved_quantity = (int)$stock_infos['reserved_quantity'];
        $physical_quantity = (int)$stock_infos['physical_quantity'];
        $available_quantity = (int)$stock_infos['quantity'];
        ...
        // Sort now multidimensional array by available quantities (get the warehouse with )
    ...
    */
                ...
                foreach ($cart->getProducts() as $row) {
                    ...
                    if (Validate::isLoadedObject($product)) {
                        ...
                        if (is_null($new_id_address_delivery) && $set_address) {
                            $new_carriers = self::getCarriersByCustomerAddresses($product, $id_warehouse, null);
                        ...
                        if ($set_address) {
                            if ($id_address_delivery > 0) {
                                ...
                                $new_carriers = self::getAvailableCarrierList($product, $id_warehouse, $id_address_delivery);
    ...
    public static function getDeliveryAddressOfSelectedWarehouse($product, $id_warehouse, $id_address_delivery = null)
    {
        ...
        if (!empty($id_address_delivery)) {
            $carriers = self::getAvailableCarrierList($product, $id_warehouse, $id_address_delivery);
            if (count($carriers)) {
    ...
    public static function getCarriersByCustomerAddresses(
        ...
        if (Configuration::get('WKWAREHOUSE_ALLOW_MULTICARRIER_CART')) {
            if (($context->customer->isLogged() && Validate::isLoadedObject($context->customer)) || // Connected
                (!$context->customer->isLogged() && isset($context->cart) && !empty($context->cart->id_customer))) { // Not Connected
    ...
    public static function getAvailableWarehouseAndCartQuantity(
        ...
        if (Validate::isLoadedObject($product) && $product->advanced_stock_management) {
            ...
            if (Validate::isLoadedObject($cart)) {
                ...
                if ($cart_product && !$orginal_qty) {
                    $cart_quantity = isset($cart_product['quantity']) ? (int)$cart_product['quantity'] : 0;
                }
                ...
                if ($warehouses_infos) {
                    ...
                    if (!empty($warehouses_info)) {
                        foreach ($warehouses_info as $info) {/* loop all sorted warehouses (by stock) to look for the one with carrier */
                            ...
                            $available_quantity = (int)$info['available_quantity'] - $cart_quantity;

                            if ((!$allow_oosp && $available_quantity > 0) || $allow_oosp) {
            ...
            if (empty($id_warehouse)) {
                ...
                if (empty($id_warehouse)) {/* not yet associated to any warehouse ? */
                    ...
                    $selected_warehouse['quantity'] -= $cart_quantity;
            ...
            if (!empty($id_warehouse)) {
                if (!isset($warehouses_infos)) {
                    ...
                    $available_quantity -= $cart_quantity;

                    /* to get right carriers and delivery address */
                    $available_carriers = self::getCarriersByCustomerAddresses($product, $id_warehouse, $id_address_delivery);
                ...
                if ((!$allow_oosp && isset($available_quantity) && $available_quantity >= 0) || $allow_oosp) {
                    // As long as customer not yet create any delivery address OR user is guest, allow adding to cart
                    $selected_warehouse['quantity'] = isset($available_quantity) ? (int)$available_quantity : 0;
        ...
        if (Validate::isLoadedObject($cart) && $cart->nbProducts()) {
            foreach ($cart->getProducts() as $row) {
                ...
                if (Validate::isLoadedObject($product)) {
                    ...
                    if (empty(self::getAvailableCarrierList($product, $id_warehouse))) {
    ...
    */
    ...
    public static function warehousesDataOnProductPage(
        ...
        if (!Configuration::get('WKWAREHOUSE_SHOW_OUTOFSTOCK') && count($warehouses_infos)) {
    ...
    {
        if (StoreHouse::hasHiddenWarehouses() > 0) {
    ...
    public static function getNumberOfAsmProductsInCart($id_cart)
    {
