    ...
    public function initContent()
    {
        ...
        if (Tools::isSubmit('action')) {
            ...
            switch (Tools::getValue('action')) {
                ...
                case 'refreshDeliveryAddressBlock':
                        ...
                        if (Validate::isLoadedObject($cart)) {
                            ...
                            if ($cart->id_address_delivery) {
                                ...
                                $delivery_address = AddressFormat::generateAddress(
                                    new Address($delivery_address['id']),
                                    array(),
                                    '<br>'
                                );
                            }
                ...
                case 'assignWarehouseToProductInCart':
                    $id_warehouse = null;
                    ...
                    if (!Configuration::get('WKWAREHOUSE_ALLOW_MULTIWH_CART') && !empty($id_warehouse)) {
                        ...
                        if (!empty($isMultiWarehousesInCart)) {
                            ...
                            WarehouseStock::removeProductFromWarehouseCart($this->context->cart->id, $id_product, $id_product_attribute);
                            $this->ajaxDie(Tools::jsonEncode(array(
                    ...
                    if (!Configuration::get('WKWAREHOUSE_ALLOW_MULTI_ADDRESSES') &&
                        ...
                        (int)$selected_warehouse['id_address_delivery'] != $this->context->cart->id_address_delivery) {
                ...
                case 'getWarehouseAsCombination':
                    ...
                    if ($allow_set_warehouse || $display_best_warehouse) {
                        ...
                        if ($isProductAsm) {
                            ...
                            if (!Configuration::get('WKWAREHOUSE_ALLOW_MULTI_ADDRESSES') &&
                                $this->context->cart->id_address_delivery && !$selected_warehouse['has_carriers']) {
                                $this->ajaxDie(Tools::jsonEncode(array(
                                    'msgError' => sprintf(
                                        $this->module->l('This product can not be delivered by any carrier to %s', 'processactions'),
                                        $this->module->getCountryName($this->context->cart->id_address_delivery)
                                    ),
                                    'hasError' => true,
                                )));
                            }
                            ...
                            if ($warehouse_cart && (int)$warehouse_cart['id_warehouse'] > 0 &&
                                $this->context->cart->containsProduct($id_product, $id_product_attribute)) {
                ...
                case 'changeAddressDelivery':
                    ...
                    if (!count(WarehouseStock::getAvailableCarrierList(new Product($id_product, false), $id_warehouse, $new_id_address_delivery))) {
                ...
                case 'changeWarehouseFromDropdownListInCart':
                    ...
                    if ($isProductAsm) {
                        ...
                        if (Validate::isLoadedObject($warehouse)) {
                            ...
                            if (!Configuration::get('WKWAREHOUSE_ALLOW_MULTI_ADDRESSES') && $cart->id_address_delivery) {
