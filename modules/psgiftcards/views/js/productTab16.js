...
$(document).ready(function() {
    ...
    $(document).one('click', '#link-ModulePsgiftcards', function (e) {
        let checkDomExist = setInterval(function() {
            if (document.getElementById("giftcardSwitchTag") instanceof Element) {
                clearInterval(checkDomExist);
                psGiftCardHandleGiftCard();
            }
        }, 300);
    });
});

function psGiftCardHandleGiftCard () {
    psGiftCardGetIfProductIsTag(id_product);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data == 'tag') {
                                ...
                                $('#id_tax_rules_group').attr("disabled", true);
    ...
    function psGiftCardGetIfProductIsTag(id_product) {
        controller_url = controller_url.replace(/\amp;/g,'');
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
