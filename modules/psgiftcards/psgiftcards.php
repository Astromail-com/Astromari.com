...
class Psgiftcards extends Module
{
        ...
        foreach ($products as $product) {
            if ($this->checkIsGiftcard($product['id_product'])) { // if yes, save it
                for ($i = 0; $i < $product['cart_quantity']; ++$i) { // loop if there is more than one giftcard
                    ...
                    $this->attachMessageToOrder($customer, $order, $id_lang, $id_shop);
                    $this->sendMailConfirmation($product['id_product'], $customer, $order, $id_lang, $id_shop);
                }
            }
        }
    }

    public function attachMessageToOrder(Customer $customer, Order $order, $id_lang, $id_shop)
    {
        if (!$customer->is_guest) {
            return;
        }

        $message = 'Order has gift cards and was made by a guest. The link to the customization page is '
            . $this->generateLinkToGiftCardsPage($customer, $order);

        $customerThread = new CustomerThread();
        $customerThread->id_contact = 0;
        $customerThread->id_customer = (int) $order->id_customer;
        $customerThread->id_shop = $id_shop;
        $customerThread->id_order = $order->id;
        $customerThread->id_lang = $id_lang;
    ...
    public function sendMailConfirmation($id_product, Customer $customer, Order $order, $id_lang, $id_shop)
    {
        ...
        $data = [
            ...
            '{{giftcards_management_url}}' => $this->generateLinkToGiftCardsPage($customer, $order),
        ];
        ...
        Mail::Send($id_lang, 'giftcard_confirmation', $this->l('Gift card confirmation'), $data, $customer->email, null, null, null, null, null, $dir);
    }
    ...
    public function hookdisplayOrderConfirmation($params)
    {
        ...
        if (Validate::isLoadedObject($params['order'])) {
            // Variable on PS 1.7
            $order = $params['order'];
        } elseif (Validate::isLoadedObject($params['objOrder'])) {
            // Variable on PS 1.6
        ...
        $this->context->smarty->assign([
            'gfLink' => $this->generateLinkToGiftCardsPage($customer, $order),
            'customerIsGuest' => (bool) $customer->is_guest,
        ]);
     ...
     * @return string
     */
    private function generateLinkToGiftCardsPage(Customer $customer, Order $order)
    {
        $params = [];
        if ($customer->is_guest) {
            $params = [
        ...
        return $this->context->link->getModuleLink('psgiftcards', 'Giftcards', $params, true);
    }
}
